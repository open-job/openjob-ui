...
const openDrawer = async (row: InstanceTask) => {
  // Description
  ...
  state.descriptions.statusTag = row.statusTag;
  state.descriptions.statusLabel = row.statusLabel;
...
const loadingLog = async (row: InstanceTask, loading :number) => {
  if (state.closeStatus){
    return
  }

  let size = 200;
  let data = await instanceTaskApi.getTaskLog({
    taskId: row.taskId,
    status: row.status,
