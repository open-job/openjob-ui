<template>
  <el-form size="small">
    <el-form-item>
      <el-radio v-model="radioValue" :label="1">{{ $t('message.crontab.minute.minutesAllowedWildcard') }}</el-radio>
    </el-form-item>

    <el-form-item>
      <el-radio v-model="radioValue" :label="2">
        {{ $t('message.crontab.minute.periodStart') }}
        <el-input-number v-model="cycle01" :min="0" :max="58" /> - <el-input-number v-model="cycle02" :min="cycle01 + 1" :max="59" /> 
        {{ $t('message.crontab.index.minuteTimeUnit') }}
      </el-radio>
    </el-form-item>

    <el-form-item>
      <el-radio v-model="radioValue" :label="3">
        {{ $t('message.crontab.minute.from') }}
        <el-input-number v-model="average01" :min="0" :max="58" /> {{ $t('message.crontab.minute.minuteStartEvery') }}
        <el-input-number v-model="average02" :min="1" :max="59 - average01" /> {{ $t('message.crontab.minute.executeOncePerMinute') }}
      </el-radio>
    </el-form-item>

    <el-form-item>
      <el-radio v-model="radioValue" :label="4">
        {{ $t('message.crontab.minute.assign') }}
